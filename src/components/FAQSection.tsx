...
export function FAQSection() {
  const [openItems, setOpenItems] = useState<number[]>([1]); // First question open by default
      ...
      <div className="flex flex-col gap-8 items-center max-w-[1152px] mx-auto">
        ...
        <div className="flex flex-col items-center w-full">
          {faqData.map((item, index) => (
            <div key={item.id}>
              <div 
                className="flex gap-2 md:gap-4 items-start rounded-[16px] w-full cursor-pointer transition-all duration-300 ease-out hover:scale-[1.01] py-4 md:p-0"
                onClick={() => toggleItem(item.id)}
              >
                <div className="flex gap-2.5 items-center pt-1">
                  <div className={`relative size-[7px] md:size-[10px] transition-all duration-500 ease-in-out ${
                    ...
                    <div className="absolute inset-[-5%]">
                      <Image
                        ...
                        width={10}
                        height={10}
                        className={`block max-w-none w-[7px] h-[7px] md:w-[10px] md:h-[10px] transition-all duration-500 ${
                          openItems.includes(item.id) ? 'scale-110' : 'scale-100'
                        }`}
                        loading="lazy"
                      />
                    </div>
                ...
                <div className="flex flex-col items-start flex-1">
                  <p className={`font-jetbrains-mono text-[16px] md:text-[20px] font-normal text-[#292827] uppercase transition-colors duration-300 ${
                    openItems.includes(item.id) ? 'text-[#1a1918]' : 'text-[#292827]'
                  }`}>
                    {item.question}
                  </p>
                  <div 
                    className={`overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${
                      openItems.includes(item.id) 
                        ? 'max-h-[500px] opacity-100 translate-y-0 pt-2' 
                        : 'max-h-0 opacity-0 -translate-y-2 pt-0'
                    }`}
                  >
                    <p className="font-raleway-medium text-[16px] font-medium leading-[1.5] text-[#52504e] w-full">
                      {item.answer}
                    </p>
                  </div>
                </div>
